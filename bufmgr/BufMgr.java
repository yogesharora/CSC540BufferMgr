	...
	public void pinPage(PageId pageId, Page page, boolean emptyPage)
		...
		if (frame != null) {
			...
			if (emptyList.size() > 0) {
				...
				// get empty frame number
				int frameNo = emptyList.get(0).intValue();
				emptyList.remove(0);
				// get associated frame data
				byte[] frameData = buffer[frameNo];
				// create pagetable entry
				frameTable[frameNo] = new BufMgrFrameDesc(pageId, frameData,
						frameNo);
				frame = frameTable[frameNo];
				createPageTableEntry(pageId, page, emptyPage, frame);

			} else {
				// we need a victim frame
				int victimFrameNo = replacer.pick_victim();
				...
				frameTable[victimFrameNo] = new BufMgrFrameDesc(pageId,
						victimFrame.getData(), victimFrameNo);
	...
	public void freePage(PageId pageId) throws InvalidBufferException,
		...
		if (frame != null) {
			...
			if (pinCount > 1) {
				...
				emptyList.add(new Integer(frame.getFrameNumber()));
